...
outdir <- "/projects/nforde/HCP/stats/OASIS/matched"
...
FS_full <- select(FS_orig, -"Education") %>% merge(FS_SA, by.x=c("Subject","FS_FSDATA.ID"), by.y=c("subject", "assessor"))
...
behav <- filter(behaviour, dx1 == "Cognitively normal") 

all <- merge(FS, behav[c("Subject", "cdr", "ageAtEntry", "dx1")], by = "Subject") %>% distinct(Subject, dx1, .keep_all = TRUE)
all$age_diff <- (all$Age - all$ageAtEntry)

##match groups
all$treat <- ifelse(all$Sex == "M", 1, 0)
match.it <- matchit(treat ~ Age, data=all)
all <- match.data(match.it)
