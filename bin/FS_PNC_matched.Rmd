...
outdir <- "/projects/nforde/HCP/stats/PNC/matched"
...
#match groups!
sub <- demogs[c("SUBJID", "Sex", "age_at_cnb")]
    
sub$treat <- ifelse(sub$Sex == "M", 1, 0)
match.it <- matchit(treat ~ age_at_cnb, data=sub)
demogs <- match.data(match.it)
...
Vol.df <- FS %>% dplyr::select(contains("vol")) %>% cbind(FS["ID"]) 
colnames(Vol.df) <- sub("_volume", "", colnames(Vol.df)) 
colnames(Vol.df) <- sub("Vol", "", colnames(Vol.df)) 

#select subcortical structures
Vol.R <- FS %>% dplyr::select(starts_with("Right")) %>% cbind(FS["ID"])
Vol.L <- FS %>% dplyr::select(starts_with("Left")) %>% cbind(FS["ID"])
...
## Cortical thickness
CT.df <- FS %>% dplyr::select(ends_with("thickness")) %>% cbind(FS["ID"]) 
colnames(CT.df) <- sub("_thickness", "", colnames(CT.df)) 

CT.R <- CT.df %>% dplyr::select(starts_with("rh_")) %>% cbind(FS["ID"])
colnames(CT.R) <- sub("rh_", "", colnames(CT.R)) 
CT.R$hemi <- "right" 

CT.L <- CT.df %>% dplyr::select(starts_with("lh_")) %>% cbind(FS["ID"])
...
### Surface Area
SA.df <- FS %>% dplyr::select(ends_with("area")) %>% cbind(FS["ID"])
colnames(SA.df) <- sub("_area", "", colnames(SA.df)) 

SA.R <- SA.df %>% dplyr::select(starts_with("rh_")) %>% cbind(FS["ID"])
colnames(SA.R) <- sub("rh_", "", colnames(SA.R)) 
SA.R$hemi <- "right" 

SA.L <- SA.df %>% dplyr::select(starts_with("lh_")) %>% cbind(FS["ID"])
