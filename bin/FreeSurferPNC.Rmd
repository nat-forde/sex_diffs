...
old_pheno <- read.csv("/projects/nforde/PNC/phs000607.v1.pht003445.v1.p1.c1.Neurodevelopmental_Genomics_Subject_Phenotypes.GRU-NPU.txt",, sep='\t', comment.char="#", header=TRUE)
...
plot_vio <- function(ROIs, df) {
  a <- lapply(ROIs, function(f) ggplot(df, aes(x="Sex", fill= Sex)) + geom_split_violin(aes_string(y=f)) 
              ...
              +stat_summary(aes_string(y=f), fun.data=mean_sdl, fun.args = list(mult = 1), geom="pointrange", shape =95, fatten =10,
...
plot_vio_hemi <- function(ROIs, df) {
  a <- lapply(ROIs, function(f) ggplot(df, aes(x=hemi, fill = Sex)) + geom_split_violin(aes_string(y=f)) 
              + theme(axis.title.x = element_blank(), axis.text=element_text(size=8), axis.title=element_text(size=10), legend.position="none") 
              +stat_summary(aes_string(y=f), fun.data=mean_sdl, fun.args = list(mult = 1), geom="pointrange", shape =95, fatten=10, size=0.5, 
...
Vol.Z <- Vol.res %>% select(overall) %>% apply(2, scale, scale = TRUE) %>%
  cbind(Vol.res[c("Sex", "age_at_cnb", "ID")]) 
...
All.resAgeOUT <- merge(Vol.resZ, SC.resZ, by="ID") %>% merge(CT.resZ, by.x="ID", by.y="CT_ID") %>% 
  merge(SA.resZ, by.x="ID", by.y="SA_ID") %>% 
  select(contains("age.resid")) %>% cbind(Vol.resZ[c("ID", "Sex", "age_at_cnb")]) 

AllresAgeOUT.file <- paste(outdir, "AllresAgeOUT.csv", sep="/")
write.csv(All.resAgeOUT, file=AllresAgeOUT.file, row.names = T)

colnames(CT.Z) <- paste0("CT_", colnames(CT.Z))
colnames(SA.Z) <- paste0("SA_", colnames(SA.Z))

All.Z <- merge(Vol.Z, SC.Z, by=c("ID", "Sex", "age_at_cnb")) %>% merge(CT.Z, by.x=c("ID", "Sex", "age_at_cnb"), by.y=c("CT_ID", "CT_Sex", "CT_age_at_cnb")) %>% 
  merge(SA.Z, by.x=c("ID", "Sex", "age_at_cnb"), by.y=c("SA_ID", "SA_Sex", "SA_age_at_cnb"))
...
PNC_SCraw <- ggseg(SCplot_sig.raw, atlas="aseg", mapping=aes(fill=as.numeric(AgeResid_Cohen)), colour="black", size=0.4) +
  ...
  theme(legend.position="none", axis.title = element_blank(), axis.text= element_blank(), line=element_blank())
...
##CT
PNC_CTraw <- ggseg(CTplot_sig.raw, mapping=aes(fill=as.numeric(AgeResid_Cohen)), colour="black", size=0.4, position="stacked") +
  scale_fill_gradient2(low="#0033FF", mid="white", high="#FF3300", midpoint=0, limits=c(-1.4, 1.4), breaks=seq(-1,1,by=1)) + 
  theme(legend.position="none", axis.title = element_blank(), axis.text= element_blank(), line=element_blank())
PNC_CTcor <- ggseg(CTplot_sig.cor, mapping=aes(fill=as.numeric(FullResid_Cohen)), colour="black", size=0.4, position="stacked") +
  scale_fill_gradient2(low="#0033FF", mid="white", high="#FF3300", midpoint=0, limits=c(-1.4, 1.4), breaks=seq(-1,1,by=1)) + 
  theme(legend.position="none", axis.title = element_blank(), axis.text= element_blank(), line=element_blank())

PNC_CTrawVR <- ggseg(CTplotVR_sig.raw, mapping=aes(fill=as.numeric(VR_Age_VR)), colour="black", size=0.4, position="stacked") +
  scale_fill_gradient2(low="#006600", mid="#FFFF99", high="#FF9900", midpoint=1.8, limits=c(1, 2.81), breaks=seq(1,2.81,by=0.5))  + 
  theme(legend.position="none", axis.title = element_blank(), axis.text= element_blank(), line=element_blank())
PNC_CTcorVR <- ggseg(CTplotVR_sig.cor, mapping=aes(fill=as.numeric(VR_Full_VR)), colour="black", size=0.4, position="stacked") +
  scale_fill_gradient2(low="#006600", mid="#FFFF99", high="#FF9900", midpoint=1.8, limits=c(1, 2.81), breaks=seq(1,2.81,by=0.5))  + 
  theme(legend.position="none", axis.title = element_blank(), axis.text= element_blank(), line=element_blank())

##SA
PNC_SAraw <- ggseg(SAplot_sig.raw, mapping=aes(fill=as.numeric(AgeResid_Cohen)), colour="black", size=0.4, position="stacked") +
  scale_fill_gradient2(low="#0033FF", mid="white", high="#FF3300", midpoint=0, limits=c(-1.4, 1.4), breaks=seq(-1,1,by=0.5)) + 
  theme(legend.position="none", axis.title = element_blank(), axis.text= element_blank(), line=element_blank())
PNC_SAcor <- ggseg(SAplot_sig.cor, mapping=aes(fill=as.numeric(FullResid_Cohen)), colour="black", size=0.4, position="stacked") +
  scale_fill_gradient2(low="#0033FF", mid="white", high="#FF3300", midpoint=0, limits=c(-1.4, 1.4), breaks=seq(-1,1,by=0.5)) + 
  theme(legend.position="none", axis.title = element_blank(), axis.text= element_blank(), line=element_blank())

PNC_SArawVR <- ggseg(SAplotVR_sig.raw, mapping=aes(fill=as.numeric(VR_Age_VR)), colour="black", size=0.4, position="stacked") +
  scale_fill_gradient2(low="#006600", mid="#FFFF99", high="#FF9900", midpoint=1.8, limits=c(1, 2.81), breaks=seq(1,2.81,by=0.5))  + 
  theme(legend.position="none", axis.title = element_blank(), axis.text= element_blank(), line=element_blank())
PNC_SAcorVR <- ggseg(SAplotVR_sig.cor, mapping=aes(fill=as.numeric(VR_Full_VR)), colour="black", size=0.4, position="stacked") +
  scale_fill_gradient2(low="#006600", mid="#FFFF99", high="#FF9900", midpoint=1.8, limits=c(1, 2.81), breaks=seq(1,2.81,by=0.5))  + 
  theme(legend.position="none", axis.title = element_blank(), axis.text= element_blank(), line=element_blank())
