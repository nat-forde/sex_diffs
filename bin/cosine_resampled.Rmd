...
```{r Cosine Angle with resampled age distributions}
...
#down-sample to normal age distribution (these steps will hopefully result in similar normally distributed age distributions in both males and females)
HCP.n <- round(nrow(HCP.unit) *0.8)
HCP.mm <- mean(HCP.unit$Age)
HCP.sdev <- 2.5
HCP.weights <- dnorm(HCP.unit$Age, mean=HCP.mm, sd=HCP.sdev)
HCP.resamp <- sample(HCP.unit, size= HCP.n, prob=HCP.weights)

PNC.n <- round(nrow(PNC.unit) *0.8)
PNC.mm <- mean(PNC.unit$Age)
PNC.sdev <- 2.5
PNC.weights <- dnorm(PNC.unit$Age, mean=PNC.mm, sd=PNC.sdev)
PNC.resamp <- sample(PNC.unit, size= PNC.n, prob=PNC.weights)

OASIS.n <- round(nrow(OASIS.unit) *0.8)
OASIS.mm <- mean(OASIS.unit$Age)
OASIS.sdev <- 2.5
OASIS.weights <- dnorm(OASIS.unit$Age, mean=OASIS.mm, sd=OASIS.sdev)
OASIS.resamp <- sample(OASIS.unit, size= OASIS.n, prob=OASIS.weights)


#make unit vectors
#HCP
HCP.unit <- apply(HCP.resamp[c(1:(length(HCP.resamp)-6))], 1, unit1) %>% t() %>% cbind(HCP.resamp[c("Age", "Subject", "Sex")])
...
## PNC
PNC.unit <- apply(PNC.resamp[c(1:(length(PNC.resamp)-6))], 1, unit1) %>% t() %>% cbind(PNC.resamp[c("Age", "ID", "Sex")])
...
## OASIS
OASIS.unit <- apply(OASIS.resamp[c(1:(length(OASIS.resamp)-6))], 1, unit1) %>% t() %>% cbind(OASIS.resamp[c("Age", "Subject", "Sex")])
