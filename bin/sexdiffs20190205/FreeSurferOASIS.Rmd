...
SubCortLR <- c("lh_Thalamus.Proper", "lh_Caudate", "lh_Putamen", "lh_Pallidum", "lh_Hippocampus", "lh_Amygdala", "lh_Accumbens.area", "rh_Thalamus.Proper", "rh_Caudate", "rh_Putamen",  "rh_Pallidum", "rh_Hippocampus",  "rh_Amygdala", "rh_Accumbens.area")
...
OV_to_graph_age <- c("BrainSeg_No_Vent", "TotalGrayage.resid", "CorticalWhiteMatterage.resid", "Cerebellum_Cortage.resid", "Cerebellum_WMage.resid") #########
...
subCort_hemi  <- lapply(SubCort, function(i){
  lmer(get(i) ~ Age + Sex*hemi + (1|Subject), data=Vol.hemi)
})
...
SC.R.res <- SC.res %>% select(starts_with("rh_")) %>% select(ends_with("resid")) %>% cbind(SC.res[1])
colnames(SC.R.res) <- sub("rh_", "", colnames(SC.R.res)) 
SC.R.res$hemi <- "right" 

SC.L.res <- SC.res %>% select(starts_with("lh_")) %>% select(ends_with("resid")) %>% cbind(SC.res[1])
...
CortCT_hemi  <- lapply(Cort, function(i){
  lmer(get(i) ~ Age + Sex*hemi + (1|Subject), data=CT.hemi)
})
...
CT.R.res <- CT.res %>% select(starts_with("rh_")) %>% cbind(CT.res[1])
colnames(CT.R.res) <- sub("rh_", "", colnames(CT.R.res)) 
CT.R.res$hemi <- "right" 

CT.L.res <- CT.res %>% select(starts_with("lh_")) %>% cbind(CT.res[1])
...
CortSA_hemi  <- lapply(Cort, function(i){
  lmer(get(i) ~ Age + Sex*hemi + (1|Subject), data=SA.hemi)
})
...
SA.R.res <- SA.res %>% select(starts_with("rh_")) %>% cbind(SA.res[1])
colnames(SA.R.res) <- sub("rh_", "", colnames(SA.R.res)) 
SA.R.res$hemi <- "right" 

SA.L.res <- SA.res %>% select(starts_with("lh_")) %>% cbind(SA.res[1])
...
All.res <- merge(Vol.res, SC.res, by="Subject") %>% merge(CT.res, by.x="Subject", by.y="CT_Subject") %>% 
  ...
  select(contains("Full.resid")) %>% cbind(Vol.res[c("Subject", "Age", "Sex")]) 
...
OASIS_CTrawAGExSEX <- ggseg(CTplotX_sig.raw, mapping=aes(fill=as.numeric(simpX_F_XTBVheight)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 30), breaks=seq(0,30,by=15)) + theme(legend.position="none", axis.title.x = element_blank()) 
OASIS_CTcorAGExSEX <- ggseg(CTplotX_sig.cor, mapping=aes(fill=as.numeric(TBVX_F_XTBVheight)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 30), breaks=seq(0,30,by=15)) + theme(legend.position="none", axis.title.x = element_blank()) 

OASIS_CTrawAGE <- ggseg(CTplotAGE_sig.raw, mapping=aes(fill=as.numeric(simp_F_age)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 600), breaks=seq(0,600,by=100)) + theme(legend.position="none", axis.title.x = element_blank()) 
OASIS_CTcorAGE <- ggseg(CTplotAGE_sig.cor, mapping=aes(fill=as.numeric(TBV_F_age)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 600), breaks=seq(0,600,by=100)) + theme(legend.position="none", axis.title.x = element_blank()) 
...
OASIS_SArawAGE <- ggseg(SAplotAGE_sig.raw, mapping=aes(fill=as.numeric(simp_F_age)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 120), breaks=seq(0,120,by=30)) + theme(legend.position="none", axis.title.x = element_blank())
OASIS_SAcorAGE <- ggseg(SAplotAGE_sig.cor, mapping=aes(fill=as.numeric(TBV_F_age)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 120), breaks=seq(0,120,by=30)) + theme(legend.position="none", axis.title.x = element_blank()) 

OASIS_SArawAGExSEX <- ggseg(SAplotX_sig.raw, mapping=aes(fill=as.numeric(simpX_F_XTBVheight)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 15), breaks=seq(0,15,by=5)) + theme(legend.position="none", axis.title.x = element_blank())
OASIS_SAcorAGExSEX <- ggseg(SAplotX_sig.cor, mapping=aes(fill=as.numeric(TBVX_F_XTBVheight)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 15), breaks=seq(0,15,by=5)) + theme(legend.position="none", axis.title.x = element_blank()) 
...
OASIS_OVplots_age.res <- OVplots_age.res
