...
SubCortLR <- c("Left.Thalamus.Proper", "Left.Caudate", "Left.Putamen", "Left.Pallidum", "Left.Hippocampus", "Left.Amygdala", "Left.Accumbens.area", "Right.Thalamus.Proper", "Right.Caudate", "Right.Putamen",  "Right.Pallidum", "Right.Hippocampus",  "Right.Amygdala", "Right.Accumbens.area")
...
TBVsimpX <- lm(BrainSegNotVent ~ age_at_cnb*Sex, data=Vol.df)
TBVsimp <- lm(BrainSegNotVent ~ age_at_cnb + Sex, data=Vol.df)
TBVageres <- lm(BrainSegNotVent ~ age_at_cnb, data=Vol.df)
...
overall_simpX  <- lapply(overall, function(i){
  lm(get(i) ~ age_at_cnb*Sex, data=Vol.df)
})
## overall simple
...
OVres_age_ls <-Vol.res %>% dplyr::select(contains("age.resid")) %>% names
OVres_TBV_ls <-Vol.res %>% dplyr::select(contains("TBV.resid")) %>% names
OVres_full_ls <-Vol.res %>% dplyr::select(contains("Full.resid")) %>% names

overall_age  <- lapply(OVres_age_ls, function(i){
  t.test(get(i) ~ Sex , data=Vol.res)
})
overall_TBV  <- lapply(OVres_TBV_ls, function(i){
  lm(get(i) ~ age_at_cnb + Sex , data=Vol.res)
})
...
```{r}
# raw model
Vol.M <- subset(Vol.res, Sex == "M")
Vol.F <- subset(Vol.res, Sex == "F")

TBVageres.M <- subset(TBVageres.df, Sex == "M")
TBVageres.F <- subset(TBVageres.df, Sex == "F")

# for TBV only
TBV_var <- var.test(Vol.M$BrainSegNotVent, Vol.F$BrainSegNotVent)
TBV_est <- TBV_var$estimate
TBV_VRp <- TBV_var$p.value
TBV_VRpadj <- p.adjust(TBV_VRp, method="fdr", n=5) ########

TBVres_var <- var.test(TBVageres.M$BrainSegNotVent, TBVageres.F$BrainSegNotVent)
...
OV_to_graph_age <- c("BrainSegNotVent", "TotalGrayage.resid", "CorticalWhiteMatterage.resid", "Cerebellum_Cortage.resid", "Cerebellum_WMage.resid") ########
...
OV_to_graph_TBV <- c("BrainSegNotVent", OVres_TBV_ls) 
OV_to_graph_full <- c("BrainSegNotVent", OVres_full_ls) 
...
SCres_age_ls <-SC.res %>% dplyr::select(contains("age.resid")) %>% names
SCres_TBV_ls <-SC.res %>% dplyr::select(contains("TBV.resid")) %>% names
SCres_full_ls <-SC.res %>% dplyr::select(contains("Full.resid")) %>% names

subCort_age  <- lapply(SCres_age_ls, function(i){
  t.test(get(i) ~ Sex , data=SC.res)
})
subCort_TBV  <- lapply(SCres_TBV_ls, function(i){
  lm(get(i) ~ age_at_cnb + Sex , data=SC.res)
})
...
subCort_hemi  <- lapply(SubCort, function(i){
  lmer(get(i) ~ age_at_cnb + Sex*hemi + (1|ID), data=Vol.hemi)
})
...
```{r}
# raw model
# SC.res.M <- subset(SC.res, Sex == "M")
# SC.res.F <- subset(SC.res, Sex == "F")
SC.M.res <- subset(SC.res, Sex == "M") ##SC.res made above includes residuals 
SC.F.res <- subset(SC.res, Sex == "F")
...
SC.R.res <- SC.res %>% dplyr::select(starts_with("Right.")) %>% cbind(SC.res[1])
colnames(SC.R.res) <- sub("Right.", "", colnames(SC.R.res)) 
SC.R.res$hemi <- "right" 

SC.L.res <- SC.res %>% dplyr::select(starts_with("left.")) %>% cbind(SC.res[1])
...
SC_to_graph_TBV <-SC.hemi.res %>% dplyr::select(contains("TBV.resid")) %>% names
...
CortCT_simpX  <- lapply(CortLR, function(i){
  lm(get(i) ~ age_at_cnb*Sex, data=CT.df)
})
...
CTres_age_ls <-CT.res %>% dplyr::select(contains("age.resid")) %>% names
CTres_TBV_ls <-CT.res %>% dplyr::select(contains("TBV.resid")) %>% names
CTres_full_ls <-CT.res %>% dplyr::select(contains("Full.resid")) %>% names

CT_age  <- lapply(CTres_age_ls, function(i){
  t.test(get(i) ~ Sex , data=CT.res)
})
CortCT_TBV  <- lapply(CTres_TBV_ls, function(i){
  lm(get(i) ~ age_at_cnb + Sex , data=CT.res)
})
...
CortCT_hemi  <- lapply(Cort, function(i){
  lmer(get(i) ~ age_at_cnb + Sex*hemi + (1|ID), data=CT.hemi)
})
...
```{r}
CT.M <- subset(CT.df, Sex == "M") # 
CT.F <- subset(CT.df, Sex == "F")
...
CT.M.res <- subset(CT.res, Sex == "M") # 
CT.F.res <- subset(CT.res, Sex == "F")
...
CT.R.res <- CT.res %>% dplyr::select(starts_with("rh_")) %>% cbind(CT.res[1])
colnames(CT.R.res) <- sub("rh_", "", colnames(CT.R.res)) 
CT.R.res$hemi <- "right" 

CT.L.res <- CT.res %>% dplyr::select(starts_with("lh_")) %>% cbind(CT.res[1])
...
CortSA_simpX  <- lapply(CortLR, function(i){
  lm(get(i) ~ age_at_cnb*Sex, data=SA.df)
})
...
SAres_age_ls <-SA.res %>% dplyr::select(contains("age.resid")) %>% names
SAres_TBV_ls <-SA.res %>% dplyr::select(contains("TBV.resid")) %>% names
SAres_full_ls <-SA.res %>% dplyr::select(contains("Full.resid")) %>% names

SA_age  <- lapply(SAres_age_ls, function(i){
  t.test(get(i) ~ Sex , data=SA.res)
})
CortSA_TBV  <- lapply(SAres_TBV_ls, function(i){
  lm(get(i) ~ age_at_cnb + Sex , data=SA.res)
})
...
CortSA_hemi  <- lapply(Cort, function(i){
  lmer(get(i) ~ age_at_cnb + Sex*hemi + (1|ID), data=SA.hemi)
})
...
# SA variance testing
```{r}
SA.M <- subset(SA.df, Sex == "M") # 
SA.F <- subset(SA.df, Sex == "F")
...
SA.M.res <- subset(SA.res, Sex == "M") # 
SA.F.res <- subset(SA.res, Sex == "F")
...
SA.R.res <- SA.res %>% dplyr::select(starts_with("rh_")) %>% cbind(SA.res[1])
colnames(SA.R.res) <- sub("rh_", "", colnames(SA.R.res)) 
SA.R.res$hemi <- "right" 

SA.L.res <- SA.res %>% dplyr::select(starts_with("lh_")) %>% cbind(SA.res[1])
...
All.res <- merge(Vol.res, SC.res, by="ID") %>% merge(CT.res, by.x="ID", by.y="CT_ID") %>% 
  merge(SA.res, by.x="ID", by.y="SA_ID") %>% 
  select(contains("Full.resid")) %>% cbind(Vol.res[c("ID", "Sex", "age_at_cnb")]) 
...
PNC_CTrawAGExSEX <- ggseg(CTplotX_sig.raw, mapping=aes(fill=as.numeric(simpX_F_XTBVheight)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 30), breaks=seq(0,30,by=15)) + theme(legend.position="none", axis.title.x = element_blank()) 
PNC_CTcorAGExSEX <- ggseg(CTplotX_sig.cor, mapping=aes(fill=as.numeric(TBVX_F_XTBVheight)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 30), breaks=seq(0,30,by=15)) + theme(legend.position="none", axis.title.x = element_blank()) 

PNC_CTrawAGE <- ggseg(CTplotAGE_sig.raw, mapping=aes(fill=as.numeric(simp_F_age)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 600), breaks=seq(0,600,by=100)) + theme(legend.position="none", axis.title.x = element_blank()) 
PNC_CTcorAGE <- ggseg(CTplotAGE_sig.cor, mapping=aes(fill=as.numeric(TBV_F_age)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 600), breaks=seq(0,600,by=100)) + theme(legend.position="none", axis.title.x = element_blank()) 
...
PNC_SArawAGE <- ggseg(SAplotAGE_sig.raw, mapping=aes(fill=as.numeric(simp_F_age)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 120), breaks=seq(0,120,by=30)) + theme(legend.position="none", axis.title.x = element_blank())
PNC_SAcorAGE <- ggseg(SAplotAGE_sig.cor, mapping=aes(fill=as.numeric(TBV_F_age)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 120), breaks=seq(0,120,by=30)) + theme(legend.position="none", axis.title.x = element_blank()) 

PNC_SArawAGExSEX <- ggseg(SAplotX_sig.raw, mapping=aes(fill=as.numeric(simpX_F_XTBVheight)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 15), breaks=seq(0,15,by=5)) + theme(legend.position="none", axis.title.x = element_blank())
PNC_SAcorAGExSEX <- ggseg(SAplotX_sig.cor, mapping=aes(fill=as.numeric(TBVX_F_XTBVheight)), colour="black", size=0.4) +
  scale_fill_gradient(limits=c(0, 15), breaks=seq(0,15,by=5)) + theme(legend.position="none", axis.title.x = element_blank()) 
...
PNC_OVplots_age.res <- OVplots_age.res
