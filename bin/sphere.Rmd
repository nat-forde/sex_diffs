...
#   
# 
# x3.unit <- apply(HCP.norm[c(1:3)], 1, unit1) %>% t() %>% cbind(HCP.norm[c("sex.coded", "Age", "Subject", "Sex")])
# yM <- filter(x3.unit, sex.coded==1) %>% select(c(1:3)) %>% colMeans() %>% unit1() 
# yF <- filter(x3.unit, sex.coded==2) %>% select(c(1:3)) %>% colMeans() %>% unit1()
# 
# 
# rgl.open()
# 
# #rgl.spheres()
# #rgl.sphgrid()
# rgl.points(x3.unit[c(1:3)], color=x3.unit$sex.coded)
# rgl.bg(color="white")
# 
# rgl.points( 0.1244976, -0.2025884, 0.9713178,  color="blue", size=10) #mean
# rgl.points(0.5732461, 0.5646552, 0.5937621,  color="red", size=10) #from cent
# 
# 
...
# 
# rgl.lines(c(min(x3.unit$Total_GMFull.resid), max(x3.unit$Total_GMFull.resid)), c(0, 0), c(0, 0), color = "black")
# rgl.lines(c(0, 0), c(min(HCP.res$Cerebellum_CortFull.resid),max(x3.unit$Cerebellum_CortFull.resid)), c(0, 0), color = "red")
# rgl.lines(c(0, 0), c(0, 0), c(min(x3.unit$Tot_WMFull.resid),max(x3.unit$Tot_WMFull.resid)), color = "green")
# 
# rgl.close()
# 
# x3.M <- subset(x3.unit, Sex =="Male")
# x3.F <- subset(x3.unit, Sex =="Female")
# 
# x3.M$cos <- filter(x3.unit, sex.coded ==1) %>% select(1:3) %>% cos.dist()
# x3.F$cos <- filter(x3.unit, sex.coded ==2) %>% select(1:3) %>% cos.dist()
# x3.cos <- rbind(x3.M, x3.F)
# 
# ggplot(x3.cos, aes(x=Sex, fill= Sex, y=cos)) + geom_split_violin() +
...
#                       stat_summary(aes(y=cos), fun.data=mean_sdl, fun.args = list(mult = 1), geom="pointrange", shape =95, 
